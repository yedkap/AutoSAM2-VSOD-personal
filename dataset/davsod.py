...
import torch
import cv2
from dataset.tfs import DAVSODTransform
...
class DAVSODDataset(data.Dataset):
    ...
    def __init__(self, dir_root, train=True, sam_trans=None, cutoff=None, len_seq=4):
        ...
        self.im_h = 360
        self.im_w = 640
        self.augmentations = DAVSODTransform()
...
def get_davsod_dataset(root_dir, sam_trans=None, cutoff_eval=None, len_seq=4):
    ...
    ds_train = DAVSODDataset(dir_root_train, sam_trans=sam_trans, len_seq=len_seq)
    dir_root_val = os.path.join(root_dir, 'DAVSOD/Validation Set/')
    ds_test = DAVSODDataset(dir_root_val, train=False, sam_trans=sam_trans, cutoff=cutoff_eval, len_seq=np.inf)
