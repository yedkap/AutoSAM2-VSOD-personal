...
def sam_call(batched_input, sam, dense_embeddings, device):
    ...
    input_points = None
    input_labels = None
    mask_frame = out_mask_logits_stage_1[0].to(device=device)
    _, out_objs_ids_frame, _ = sam.add_new_mask(
        inference_state=inference_state,
        frame_idx=0,
        obj_id=0,
        mask=mask_frame,
    ...
    # out_mask_logits_points = [out_mask_logits_frame_0]
    for frame_idx in range(1, num_frames):
        mask_frame = out_mask_logits_stage_1[frame_idx].to(device=device)
        # dense_embeddings_frame = None
        # input_points = np.array([[[(W // 2 + 100), (H * (360 / 640)) // 2 - 100]] for _ in range(bs)]) #  cat batch_size
        # input_labels = np.array([[1] for _ in range(bs)]) #  cat batch_size
        with torch.no_grad():
            _, out_objs_ids_frame, out_mask_logits_frame = sam.add_new_mask(
            inference_state=inference_state,
            frame_idx=frame_idx,
            obj_id=0,
            mask=mask_frame,
